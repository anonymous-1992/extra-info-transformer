...
def objective(trial):
    ...
    if "extra_info_attn" in args.attn_type:
        ...
        n_ext_info = 1
        kernel_b = trial.suggest_categorical("kernel_b", [log_b_size, log_b_size*4, log_b_size*8])
        kernel_s = trial.suggest_categorical("kernel_s", [log_s_size, log_s_size*4, log_s_size*8])
    else:
        n_ext_info = 0
        kernel_s = 1
        kernel_b = 1
    if [d_model, n_ext_info, kernel_s, kernel_b] in param_history or n_distinct_trial > 4:
        ...
        n_distinct_trial += 1
    param_history.append([d_model, n_ext_info, kernel_s, kernel_b])
