...
class ScaledDotProductAttention(nn.Module):
    ...
    def __init__(self, d_k, device, n_heads, l_k, b_size, n_ext_info,
        ...
        if "extra_info_attn" in self.attn_type:
            ...
            self.n_linear = nn.Parameter(torch.rand(self.n*self.m).to(device), requires_grad=True)
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        if self.attn_type == "basic_attn" or not self.enc_attn:
            ...
            n = K_e.shape[2]
            K_l = torch.einsum('bhnd,n->bhnd', K_e, self.n_linear) + \
                  torch.einsum('bhnd,n->bhnd', K[:, :, -n:, :].clone(), 1 - self.n_linear)
            V_l = torch.einsum('bhnd,n->bhnd', V_e, self.n_linear) + \
                  torch.einsum('bhnd,n->bhnd', V[:, :, -n:, :].clone(), 1 - self.n_linear)
            K[:, :, -n:, :] = K_l
            V[:, :, -n:, :] = V_l
            scores = torch.einsum('bhqd,bhkd-> bhqk', Q, K) / np.sqrt(self.d_k)
            attn = self.softmax(scores)
